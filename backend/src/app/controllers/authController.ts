...
// POST /api/auth/google (connexion avec token direct - pour mobile/SPA)
export const connexionGoogle = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { idToken } = req.body;
    if (!idToken) {
      res.status(400).json({ success: false, message: 'idToken requis.' });
      return;
    }
...
// 🆕 GET /api/auth/google/url - Générer l'URL de redirection Google
...
// 🆕 GET /api/auth/google/callback - Callback après redirection Google
export const googleCallback = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const frontendUrl = `${process.env.FRONTEND_URL}/auth/google-callback?token=${result.token}&user=${encodeURIComponent(JSON.stringify(result.user))}&profilIncomplet=${result.profilIncomplet}`;
...
// GET /api/auth/verifier-token-reset/:token
...
export const reinitialiserMotDePasse = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await authService.reinitialiserMotDePasse({ 
      token, 
      email, 
      motDePasse, 
