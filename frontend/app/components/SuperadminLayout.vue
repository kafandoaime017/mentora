<template>
  <div class="flex min-h-screen bg-gray-50 font-body" v-if="authChecked">

    <!-- SIDEBAR DESKTOP -->
    <aside
      class="hidden md:flex fixed left-0 top-0 bottom-0 bg-[#0f172a] flex-col p-3 z-40 overflow-hidden shadow-xl transition-all duration-300"
    ...
    >
      ...
      <button
        ...
        class="self-end w-7 h-7 rounded-lg bg-white/10 text-white hover:bg-white/20 flex items-center justify-center cursor-pointer mb-5 flex-shrink-0"
      >
      ...
      <nav class="flex flex-col flex-1 w-full gap-1">
        <nuxt-link
          ...
          class="flex items-center gap-2 px-2 py-2 rounded-lg text-white/60 hover:bg-white/10 hover:text-white transition-all w-full"
          :class="{ 'bg-white/15 !text-white': $route.path === item.to || $route.path.startsWith(item.to + '/'), 'justify-center': sidebarCollapsed }"
          :title="item.label"
        >
      ...
      <div class="border-t border-white/10 pt-2 mt-2 w-full space-y-1">
        <nuxt-link
          to="/superadmin/profile"
          class="flex items-center gap-2 px-2 py-2 rounded-lg text-white/60 hover:bg-white/10 hover:text-white transition-all w-full"
          :class="{ 'bg-white/15 !text-white': $route.path === '/superadmin/profile', 'justify-center': sidebarCollapsed }"
          title="Mon profil"
        >
          <span class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0">
        ...
        <nuxt-link
          ...
          class="flex items-center gap-2 px-2 py-2 rounded-lg text-white/60 hover:bg-white/10 hover:text-white transition-all w-full"
          :class="{ 'bg-white/15 !text-white': $route.path === '/superadmin/parametres', 'justify-center': sidebarCollapsed }"
          title="Paramètres"
        >
          <span class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          @click="handleLogout"
          class="flex items-center gap-3 px-2 py-2 rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-all w-full"
    ...
    >
      <!-- HEADER -->
      <header class="bg-[#0f172a] px-4 md:px-6 h-16 flex items-center justify-between flex-shrink-0 sticky top-0 z-30">
        <div class="md:hidden">
          <img src="/images/logo-blanc.png" alt="Mentora" class="h-8"/>
        </div>
        <div class="hidden md:flex items-center gap-2">
          <span class="text-xs font-semibold text-white/40 uppercase tracking-wider">Super Admin</span>
          <span class="text-white/20">·</span>
          <span class="text-sm text-white/70 font-medium">{{ currentUser?.prenom }} {{ currentUser?.nom }}</span>
        </div>
        <div class="flex items-center gap-3">
          <span class="px-3 py-1 bg-amber-500/20 text-amber-400 text-xs font-semibold rounded-full hidden sm:block">
            Superadmin
          </span>
          <div class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center border-2 border-white/20 text-white text-xs font-bold">
    ...
    <!-- MOBILE BOTTOM NAV -->
    <nav class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0f172a] border-t border-white/10">
      <div class="bg-[#0f172a]">
        ...
        <div class="grid grid-cols-5 h-16">
          <nuxt-link v-for="item in navItems" :key="item.key" :to="item.to"
            ...
            :class="$route.path === item.to || $route.path.startsWith(item.to + '/') ? 'text-white' : 'text-white/40'"
          >
            <component :is="item.icon" class="w-5 h-5"/>
          ...
          <button @click="mobileMenuOpen = !mobileMenuOpen"
            class="flex flex-col items-center justify-center gap-0.5 transition-colors"
            :class="mobileMenuOpen ? 'text-white' : 'text-white/40'"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          <div v-if="mobileMenuOpen" class="border-t border-white/10 grid grid-cols-3 py-3 px-2 gap-2">
            <nuxt-link to="/superadmin/profile" @click="mobileMenuOpen = false"
              class="flex flex-col items-center justify-center gap-1 py-3 rounded-xl hover:bg-white/10 transition-colors"
              :class="$route.path === '/superadmin/profile' ? 'text-white' : 'text-white/50'"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
              </svg>
            ...
            <nuxt-link to="/superadmin/parametres" @click="mobileMenuOpen = false"
              class="flex flex-col items-center justify-center gap-1 py-3 rounded-xl hover:bg-white/10 transition-colors"
              :class="$route.path === '/superadmin/parametres' ? 'text-white' : 'text-white/50'"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
              </svg>
            ...
            <button @click="handleLogout"
              class="flex flex-col items-center justify-center gap-1 py-3 rounded-xl hover:bg-red-500/10 transition-colors text-red-400"
            >
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
              </svg>
...
.spinner { border: 4px solid rgba(0,0,0,0.1); border-top-color: #0f172a; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; }
