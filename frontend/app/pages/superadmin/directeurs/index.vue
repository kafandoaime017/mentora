    ...
    <div>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p class="text-sm text-gray-400 mt-1">{{ directeurs.length }} directeur(s) · {{ invitations.length }} invitation(s) en attente</p>
        </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="invitations.length > 0" class="bg-white rounded-2xl shadow-sm border border-yellow-200 overflow-hidden">
          <div class="px-5 py-3.5 border-b border-yellow-100 bg-yellow-50">
            <h3 class="text-sm font-bold text-yellow-800">Invitations en attente</h3>
          </div>
          <div class="divide-y divide-gray-50">
            <div v-for="inv in invitations" :key="inv.id" class="px-5 py-3.5 flex items-center justify-between flex-wrap gap-2">
              <div>
                <p class="text-sm font-semibold text-gray-800">{{ inv.prenom }} {{ inv.nom }}</p>
                <p class="text-xs text-gray-400">{{ inv.email }} · {{ inv.ecole || 'École inconnue' }}</p>
              </div>
              <div class="flex items-center gap-2">
                <span v-if="inv.expiree" class="text-xs font-semibold text-red-500">Expirée</span>
                <span v-else class="text-xs text-gray-400">Expire le {{ new Date(inv.expiresAt).toLocaleDateString('fr-FR') }}</span>
                <button @click="renvoyer(inv)" class="px-3 py-1.5 bg-blacky/5 text-blacky rounded-lg text-xs font-semibold hover:bg-blacky/10 transition">Renvoyer</button>
                <button @click="annuler(inv)" class="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-xs font-semibold hover:bg-red-100 transition">Annuler</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <table class="w-full" v-if="directeurs.length > 0">
            ...
            <tbody class="divide-y divide-gray-50">
              <tr v-for="d in directeurs" :key="d.id" class="hover:bg-gray-50 transition">
                ...
                <td class="px-5 py-4 text-sm text-gray-500">{{ d.ecole || '—' }}</td>
                ...
                <td class="px-5 py-4">
                  <div class="flex items-center justify-center gap-1.5 flex-wrap">
                    <button v-if="!d.isVerified" @click="renvoyerVerif(d)"
                      class="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-blacky/5 text-blacky hover:bg-blacky/10 transition"
                    ...
                    <button @click="confirmerSuppression(d)" class="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-gray-50 text-gray-500 hover:bg-red-50 hover:text-red-600 transition">Supprimer</button>
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div class="md:hidden space-y-3">
          <div v-for="d in directeurs" :key="d.id" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-300">{{ d.ecole || '—' }}</p>
                </div>
              </div>
              <span class="px-2.5 py-1 rounded-full text-xs font-semibold"
                :class="d.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              <button v-if="!d.isVerified" @click="renvoyerVerif(d)" class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-blacky/5 text-blacky">Renvoyer email</button>
              <button @click="toggleActif(d)" class="px-3 py-1.5 rounded-lg text-xs font-semibold" :class="d.isActive ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'">
                {{ d.isActive ? 'Désactiver' : 'Activer' }}
              </button>
              <button @click="confirmerSuppression(d)" class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-gray-50 text-gray-500">Supprimer</button>
            </div>
          </div>
      ...
      <div v-if="modalSuppressionVisible" class="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4" @click.self="modalSuppressionVisible = false">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
          <h3 class="font-bold text-gray-800 mb-2">Supprimer le directeur</h3>
          <p class="text-sm text-gray-500 mb-6">Êtes-vous sûr de vouloir supprimer <strong>{{ directeurASupprimer?.prenom }} {{ directeurASupprimer?.nom }}</strong> ? Cette action est irréversible.</p>
          <div class="flex gap-3">
