    ...
    <SuperadminLayout>
      <div>
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 class="text-2xl font-body font-extrabold text-gray-800">Administration</h1>
            <p class="text-sm font-body text-gray-500 mt-1">{{ superadmins.length }} superadmin(s) · {{ invitations.length }} invitation(s) en attente</p>
          </div>
        ...
        <div class="bg-[#f5f0e8] rounded-xl px-5 py-3.5 mb-6 flex items-start gap-3">
          ...
          <p class="text-xs font-body text-gray-600">Par sécurité, vous ne pouvez pas désactiver, révoquer ou supprimer votre propre compte depuis cette page.</p>
        </div>
                ...
                <tr v-for="s in superadmins" :key="s.id" class="hover:bg-gray-50 transition">
                  <td class="px-5 py-4">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        <p class="font-body font-semibold text-gray-800 text-sm">{{ s.prenom }} {{ s.nom }}</p>
                        <p v-if="s.id === moi" class="text-[10px] font-body font-semibold text-secondary">Vous</p>
                      </div>
                    </div>
                  </td>
                  <td class="px-5 py-4 text-sm font-body text-gray-500">{{ s.email }}</td>
                  ...
                  <td class="px-5 py-4">
                    ...
                    <div v-else class="text-center text-xs font-body text-gray-300">—</div>
                  </td>
                </tr>
              </tbody>
            </table>
            <div v-else class="p-16 text-center text-gray-400 font-body text-sm">Aucun superadmin</div>
          </div>

          <!-- Mobile cards -->
          <div class="md:hidden space-y-3">
            <div v-for="s in superadmins" :key="s.id" class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] p-4">
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="font-body font-semibold text-gray-800 text-sm">{{ s.prenom }} {{ s.nom }} <span v-if="s.id === moi" class="text-[10px] text-secondary">(Vous)</span></p>
                    <p class="text-xs font-body text-gray-400">{{ s.email }}</p>
                  </div>
                </div>
                <span class="px-2.5 py-1 rounded-full text-xs font-body font-semibold"
                  :class="s.isActive ? 'bg-green-600 text-white' : 'bg-red-600 text-white'">
            ...
            <div v-if="superadmins.length === 0" class="py-16 text-center text-gray-400 font-body text-sm">Aucun superadmin</div>
          </div>
        </div>
        ...
        <div v-if="modalSuppressionVisible" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" @click.self="modalSuppressionVisible = false">
          <div class="bg-white rounded-xl w-full max-w-sm p-6">
            ...
            <p class="text-sm font-body text-gray-500 text-center mb-6">Êtes-vous sûr de vouloir supprimer <strong>{{ superadminASupprimer?.prenom }} {{ superadminASupprimer?.nom }}</strong> ? Cette action est irréversible.</p>
