    ...
    <SuperadminLayout>
      ...
      <div v-else-if="ecole">
        ...
        <div class="flex items-center gap-2 mb-5">
          <nuxt-link to="/superadmin/ecoles" class="w-8 h-8 rounded-full bg-white shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] flex items-center justify-center hover:bg-gray-50 transition">
          ...
          <span class="text-xs font-body text-gray-400">Écoles</span>
          <span class="text-xs text-gray-300">/</span>
          <span class="text-xs font-body text-gray-600 font-semibold">{{ ecole.nom }}</span>
        </div>

        <!-- En-tête école -->
        <div class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] p-6 mb-6">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
              <div class="w-14 h-14 bg-blacky/10 rounded-xl flex items-center justify-center shrink-0">
                <svg class="w-7 h-7 text-blacky" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                </svg>
              </div>
              <div>
                <h1 class="text-xl font-body font-extrabold text-gray-800">{{ ecole.nom }}</h1>
                <p class="text-sm font-body text-gray-400">{{ ecole.ville || 'Ville non renseignée' }}</p>
              </div>
            </div>
          ...
          <div class="mt-5 pt-5 border-t border-gray-100 flex items-center justify-between flex-wrap gap-3">
            <div v-if="ecole.directeur" class="flex items-center gap-3">
              ...
              <div>
                <p class="text-sm font-body font-semibold text-gray-800">{{ ecole.directeur.prenom }} {{ ecole.directeur.nom }}</p>
                <p class="text-xs font-body text-gray-400">{{ ecole.directeur.email }} · {{ ecole.directeur.isVerified ? 'Vérifié' : 'En attente de vérification' }}</p>
              </div>
            </div>
            <p v-else class="text-sm font-body text-gray-400">Aucun directeur rattaché à cette école</p>

            <div class="flex gap-2">
              <button @click="ouvrirModalPlan" class="px-4 py-2 bg-blacky/5 text-blacky rounded-xl text-xs font-body font-semibold hover:bg-blacky/10 transition">
                Changer le plan
              </button>
              <button @click="toggleActive" class="px-4 py-2 rounded-xl text-xs font-body font-semibold transition"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] p-5">
            <div class="flex items-center justify-between mb-2">
              <p class="text-xs font-body font-semibold text-gray-400 uppercase">Étudiants</p>
              <p class="text-xs font-body font-bold text-gray-700">{{ ecole.stats.nbEtudiants }} / {{ ecole.limites.maxEtudiants === -1 ? '∞' : ecole.limites.maxEtudiants }}</p>
            </div>
          ...
          <div class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] p-5">
            <div class="flex items-center justify-between mb-2">
              <p class="text-xs font-body font-semibold text-gray-400 uppercase">Professeurs</p>
              <p class="text-xs font-body font-bold text-gray-700">{{ ecole.stats.nbProfs }} / {{ ecole.limites.maxProfs === -1 ? '∞' : ecole.limites.maxProfs }}</p>
            </div>
            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
          ...
          <div class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] p-5">
            <p class="text-xs font-body font-semibold text-gray-400 uppercase mb-2">Sessions</p>
            <p class="text-2xl font-body font-extrabold text-gray-800">{{ ecole.stats.nbSessions }}</p>
            <p class="text-xs font-body text-gray-400 mt-0.5">{{ ecole.limites.maxSessions === -1 ? 'Illimitées' : `Limite : ${ecole.limites.maxSessions}/mois` }}</p>
          </div>
        </div>
        ...
        <!-- Filières & classes -->
        <div v-if="tabActif === 'filieres'" class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] divide-y divide-gray-100">
          <div v-if="!ecole.filieres || ecole.filieres.length === 0" class="p-10 text-center text-gray-400 font-body text-sm">Aucune filière</div>
          <div v-for="f in ecole.filieres" :key="f.id" class="p-5">
            <p class="font-body font-semibold text-gray-800 text-sm mb-2">{{ f.nom }}</p>
            <div class="flex flex-wrap gap-2">
              <span v-for="c in f.classes" :key="c.id" class="px-2.5 py-1 bg-[#f5f0e8] text-gray-600 text-xs font-body rounded-full">{{ c.nom }}</span>
              <span v-if="!f.classes || f.classes.length === 0" class="text-xs font-body text-gray-300">Aucune classe</span>
            </div>
          </div>
        </div>

        <!-- Professeurs -->
        <div v-else-if="tabActif === 'profs'" class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] divide-y divide-gray-100">
          <div v-if="ecole.profs.length === 0" class="p-10 text-center text-gray-400 font-body text-sm">Aucun professeur</div>
          <div v-for="p in ecole.profs" :key="p.id" class="p-4 flex items-center justify-between">
            <div>
              <p class="text-sm font-body font-semibold text-gray-800">{{ p.prenom }} {{ p.nom }}</p>
              <p class="text-xs font-body text-gray-400">{{ p.email }} · {{ p.filiere || 'Sans filière' }}</p>
            </div>
        ...
        <!-- Étudiants -->
        <div v-else class="bg-white rounded-xl shadow-[1px_1px_7px_1px_rgba(0,0,0,0.08)] divide-y divide-gray-100">
          <div v-if="ecole.etudiants.length === 0" class="p-10 text-center text-gray-400 font-body text-sm">Aucun étudiant</div>
          <div v-for="e in ecole.etudiants.slice(0, 100)" :key="e.id" class="p-4 flex items-center justify-between">
            <div>
              <p class="text-sm font-body font-semibold text-gray-800">{{ e.prenom }} {{ e.nom }}</p>
              <p class="text-xs font-body text-gray-400">{{ e.email }} · {{ e.filiere || '—' }} {{ e.classe ? '· ' + e.classe : '' }}</p>
            </div>
          ...
          <p v-if="ecole.etudiants.length > 100" class="p-4 text-center text-xs font-body text-gray-400">+{{ ecole.etudiants.length - 100 }} autres étudiants non affichés</p>
        </div>
      </div>
        ...
        <div class="bg-white rounded-xl w-full max-w-sm p-6">
          ...
          <div class="space-y-2 mb-4">
            <button v-for="p in ['gratuit', 'starter', 'pro']" :key="p" @click="planChoisi = p"
              class="w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-body font-semibold transition"
              :class="planChoisi === p ? 'bg-blacky/10 text-blacky' : 'bg-input text-gray-600 hover:bg-gray-200'">
          ...
          <div class="flex gap-3">
            <button @click="modalPlanVisible = false" class="flex-1 font-body py-2.5 bg-gray-200 text-gray-600 rounded-xl text-sm font-semibold">Annuler</button>
