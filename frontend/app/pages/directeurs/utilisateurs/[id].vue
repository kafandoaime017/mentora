    ...
    <AdminLayout>
      <div class="max-w-3xl mx-auto">
        ...
        <div v-else-if="!user" class="text-center py-20">
          <p class="text-sm font-body text-black">Utilisateur introuvable</p>
        </div>
        ...
        <div v-else class="space-y-4">
          ...
          <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 class="text-sm font-body font-semibold text-black uppercase tracking-wide">Profil</h2>
            ...
            <div class="p-5 flex items-center gap-4">
              ...
              <div>
                <h1 class="text-xl font-body font-bold text-black">{{ user.prenom }} {{ user.nom }}</h1>
                <p class="text-sm font-body text-black mt-0.5">{{ user.email }}</p>
                <p class="text-xs font-body text-black mt-1">
          ...
          <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100">
              <h2 class="text-sm font-body font-semibold text-black uppercase tracking-wide">Informations académiques</h2>
            </div>
            <div class="p-5">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <p class="text-xs font-body text-black mb-1">Filière</p>
                  <p class="text-sm font-body font-medium text-black">{{ user.profil?.filiere || '—' }}</p>
                </div>
                <div v-if="user.role === 'etudiant'">
                  <p class="text-xs font-body text-black mb-1">Classe</p>
                  <p class="text-sm font-body font-medium text-black">{{ user.profil?.classe || '—' }}</p>
                </div>
                <div v-if="user.role === 'professeur'">
                  <p class="text-xs font-body text-black mb-1">Statut</p>
                  <span
                    class="text-xs font-body px-2 py-0.5 rounded font-medium"
                ...
                <div>
                  <p class="text-xs font-body text-black mb-1">Email vérifié</p>
                  <span
                    class="text-xs font-body px-2 py-0.5 rounded font-medium"
                    :class="user.isVerified ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-black'"
                  >
          ...
          <div v-if="user.role === 'etudiant' && sessions.length > 0" class="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100">
              <h2 class="text-sm font-body font-semibold text-black uppercase tracking-wide">
                Sessions passées
                <span class="ml-1 font-normal normal-case text-black">({{ sessions.length }})</span>
              </h2>
            </div>
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                  <th class="px-5 py-2.5 text-left text-xs font-body font-semibold text-black">Session</th>
                  ...
                  <th class="px-5 py-2.5 text-right text-xs font-body font-semibold text-black hidden md:table-cell">Date</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="s in sessions" :key="s.id" class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  <td class="px-5 py-3">
                    <p class="font-body font-medium text-black">{{ s.titre }}</p>
                    <p class="text-xs font-body text-black">{{ s.filiere }} · {{ s.classe }}</p>
                  </td>
                  ...
                  <td class="px-5 py-3 text-center">
                    <span
                      ...
                      :class="{
                        ...
                        'bg-gray-100 text-black':  s.statut === 'inscrit',
                      }"
                    >
          ...
          <div v-if="user.role === 'professeur' && sessions.length > 0" class="bg-white border border-gray-200 rounded-lg overflow-hidden">
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-gray-100 bg-gray-50">
                  <th class="px-5 py-2.5 text-left text-xs font-body font-semibold text-black">Session</th>
                  <th class="px-5 py-2.5 text-center text-xs font-body font-semibold text-black">Participants</th>
                  <th class="px-5 py-2.5 text-center text-xs font-body font-semibold text-black">Statut</th>
                  <th class="px-5 py-2.5 text-right text-xs font-body font-semibold text-black hidden md:table-cell">Date</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="s in sessions" :key="s.id" class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  ...
                  <td class="px-5 py-3 text-center">
                    <span class="font-body text-sm text-black">{{ s.nb_participants }}</span>
                  </td>
                  <td class="px-5 py-3 text-center">
                    <span
                      class="text-xs font-body px-2 py-0.5 rounded font-medium"
                      :class="{
                        ...
                        'bg-gray-100 text-black':   s.status === 'completed',
